...
local hl = require("plugin.status.hl")
local lua_utils = require("helper.lua")
local debounce = require("helper.lua").cached_debounce

local M = {}

---@alias StatusComponent function(StatusContext, Options): string

---@param ctx plugin.status.StatusContext
...
local function _filename(ctx, opts)
    ...
    local full_path = require("helper.path").shorten_path(
...
local _debounced_fns = {}

---@param ctx plugin.status.StatusContext
---@param opts plugin.status.StatusOptions
---@return string
function M.filename(ctx, opts)
    local key = ctx.win_id or -0.5
    if not ctx.is_current then
        key = key * -1
    end
    if not _debounced_fns[key] then
        _debounced_fns[key] = debounce(1000, _filename)
    end
    return _debounced_fns[key](ctx, opts)
end
...
function M.tabs(ctx, opts)
    ...
    for i = 1, total_tabs do
        ...
        local label, t_bufnr, t_winid = _get_tab_label(i, opts.icons.noname)
        local box_size = lua_utils.utf8_char_count(label) + 5

        label = label .. _buffer_flags(t_bufnr, t_winid)
        label = i .. ":" .. label
        if i == prev_tab then
            label = label .. "-"
        end
...
---@return string
local function _diagnostics(ctx, buf, opts, hls)
    local res = {}
...
local _debounced_global_diagnostics = debounce(1000, _diagnostics)

---@param ctx plugin.status.StatusContext
---@param opts plugin.status.StatusOptions
---@return string
function M.global_diagnostics(ctx, opts)
    return _debounced_global_diagnostics(ctx, nil, opts, opts.tabline_diagnostics.style and {
...
local _decounced_statusline_disgnostics = {}
...
function M.status_diagnostics(ctx, opts)
    ...
    if not _decounced_statusline_disgnostics[ctx.win_id or -1] then
        _decounced_statusline_disgnostics[ctx.win_id or -1] = debounce(1000, _diagnostics)
    end
    return _decounced_statusline_disgnostics[ctx.win_id or -1](ctx, ctx.buf, opts, opts.status_diagnostics.style and {
...
local function _pwd(ctx, opts)
    ...
    return require("helper.path").get_cwd_short(opts.pwd.truncation_target)
end

local _debounced_pwd = debounce(1000, _pwd)
