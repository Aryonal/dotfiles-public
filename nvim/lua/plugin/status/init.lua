...
local get_context = require("plugin.status.ctx").get_context
local comp        = require("plugin.status.components")
local lsp         = require("plugin.status.lsp")
local hl          = require("plugin.status.hl")


local M = {}

---@class plugin.status.StatusOptions
---@field [string] any future configuration options

---@type plugin.status.StatusOptions
local _opts = {
    statusline_exclude = {
        filetypes = {
            "TelescopePrompt",
...
function M.tabline()
    ...
    return
        ...
        with_padding(comp.global_diagnostics(ctx, _opts), 0, 1) ..
...
function M.setup(opts)
    ...
    require("plugin.status.hl").setup(_opts)

    vim.o.statusline = "%!v:lua.require('plugin.status').statusline()"
    vim.o.tabline = "%!v:lua.require('plugin.status').tabline()"

    vim.o.showtabline = 2 -- Always show tabline

    -- adhoc redraw
    vim.api.nvim_create_autocmd({ "DirChanged" }, {
        group = vim.api.nvim_create_augroup("lua/plugin/status/init.lua", { clear = true }),
        pattern = "*",
        callback = function()
