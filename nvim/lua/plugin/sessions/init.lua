...
function M.setup(opts)
    local util = require("plugin.sessions.utils")

    opts = opts or {}
    opts = util.merge_tbl(default_opts, opts)

    ---set up autocmds
    local group = vim.api.nvim_create_augroup("lua/plugin/sessions/init.nvim", { clear = true })
    require("plugin.sessions.cmd").autosave_on_leave(opts, group)
    require("plugin.sessions.cmd").autoload_on_enter(opts, group)

    ---set up commands
    require("plugin.sessions.cmd").create_cmd_save(opts)
    require("plugin.sessions.cmd").create_cmd_load(opts)
