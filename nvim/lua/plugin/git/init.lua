local debounce = require("helper.lua").debounce

local log_prefix = "lua/plugin/git/init.lua: "
...
---@return nil
function G:start()
    if require("helper.os").is_windows() then
        error("Windows is not supported")
        return
    end

    -- register autocmd to stop fs_event on VimLeavePre
    local au = vim.api.nvim_create_augroup("lua/plugin/git/init.lua", { clear = true })

    vim.api.nvim_create_autocmd({ "VimEnter" }, {
        group = au,
        desc = "[plugin/git] start wather",
        pattern = "*",
        callback = function()
    ...
    vim.api.nvim_create_autocmd({ "DirChanged" }, {
        group = au,
        desc = "[plugin/git] refresh wathcer",
        pattern = "*",
        callback = function()
            self:_watch(vim.uv.cwd())
        end,
...
local function git_root(path)
    ...
    while true do
        ...
        if path == "/" then
            -- print("[debug] lua/plugin/git/init.lua: not git repository: " .. orig_path)
            return nil
        end
    ...
    local success, err_name, _ = fs_event:start(
        ...
        function(err, filename, ev)
            ...
            if not ev.change and not ev.rename then
                -- print(vim.uv.now() .. "  [debug] lua/plugin/git/init.lua: fs_event: ev " .. vim.inspect(ev))
                return
            end
            if ignore_fn(filename) then
                return
            end
            -- print(vim.uv.now() .. "  [debug] lua/plugin/git/init.lua: fs_event: process " .. filename)
...
function G:_watch(path)
    ...
    -- print("[debug] lua/plugin/git/init.lua: refresh watcher: " .. root)
