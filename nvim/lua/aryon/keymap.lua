...
local global_lsp_bindings = {
    { cfg.lsp.show_diagnostics_inline, _diagnostic_float, desc = "[LSP] Diagnostics (float)" },
    { cfg.motion.buffer.diag_next,     _diagnostic_next,  desc = "[LSP] Next Diagnostic" },
    { cfg.motion.buffer.diag_prev,     _diagnostic_prev,  desc = "[LSP] Previous Diagnostic" },
}
local bindings = {}

vim.list_extend(bindings, global_lsp_bindings)

local maps = require("x.helper.vim").batch_set_keymap
maps(bindings)
