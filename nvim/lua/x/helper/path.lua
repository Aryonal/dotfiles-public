...
---shortens path by turning apple/orange -> app…/orange -> a/orange
---@param path string
...
function M.shorten_path(path, sep, max_len)
    ...
    for idx = 1, #segments - 1 do
        ...
        local truncated = math.min(len - max_len, #segment - (vim.startswith(segment, ".") and 2 or 1))
        local right_idx = #segment - truncated
        local shortened = ""
        if right_idx > 4 then
            right_idx = right_idx - 3
            shortened = segment:sub(1, right_idx) .. "…"
        else
            shortened = segment:sub(1, right_idx)
        end
